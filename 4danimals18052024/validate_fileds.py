...
def is_valid_israeli_id(id_number):
    """
    This fuction check the parameter id_number that enter as string  
    is a valid isreali id ,if not 9 digit the func complit zero on the left
    and return True/False if valdation success
    """
...
def is_filed_valid(name, value, required):
    """
    This function validate filed accordint to the name of the filed and if it is required
    return True/False if filed valid
    """
...
def validate_form(**fields):
    """
    This function get dictionary with the user input and loop all the filed and validat them
    return True/False for all filed validation , errors as string if exist
    """
    validation = True
    errors = []
